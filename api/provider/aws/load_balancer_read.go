...
func (l *LoadBalancerProvider) Read(loadBalancerID string) (*models.LoadBalancer, error) {
	...
	if loadBalancer.isCLB {
		...
		loadBalancerAttributes, err := describeCLBAttributes(l.AWS.ELB, fqLoadBalancerID)
		if err != nil {
			return nil, err
		}
	...
	if loadBalancer.isALB {
		...
		loadBalancerARN := aws.StringValue(loadBalancer.ALB.LoadBalancerArn)
		attributes, err := describeALBAttributes(l.AWS.ALB, loadBalancerARN)
		if err != nil {
			return nil, err
		}
		...
		for _, attribute := range attributes {
			if aws.StringValue(attribute.Key) == "idle_timeout.timeout_seconds" {
				timeoutString = aws.StringValue(attribute.Value)
			}
		}
...
func (l *LoadBalancerProvider) makeLoadBalancerModel(loadBalancerID string) (*models.LoadBalancer, error) {
	...
	if tag, ok := tags.WithKey("arn").First(); ok {
		model.LoadBalancerARN = tag.Value
	}
