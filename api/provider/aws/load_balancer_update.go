...
func (l *LoadBalancerProvider) Update(loadBalancerID string, req models.UpdateLoadBalancerRequest) error {
	...
	if req.IdleTimeout != nil {
		...
		if isClassicELB {
			if err := l.setCLBIdleTimeout(loadBalancerName, *idleTimeout); err != nil {
				return err
			}
		}

		if isAppLB {
			loadBalancerARN, err := lookupLoadBalancerARNFromID(l.TagStore, loadBalancerID)
			if err != nil {
				return err
			}

			if err := l.setALBIdleTimeout(loadBalancerARN, *idleTimeout); err != nil {
				return err
			}
		}
	}
...
func (l *LoadBalancerProvider) setALBIdleTimeout(loadBalancerARN string, idleTimeout int) error {
	attribute := &alb.LoadBalancerAttribute{}
	attribute.SetKey("idle_timeout.timeout_seconds")
	attribute.SetValue(fmt.Sprintf("%d", idleTimeout))

	attributes := []*alb.LoadBalancerAttribute{attribute}

	input := &alb.ModifyLoadBalancerAttributesInput{}
	...
	if _, err := l.AWS.ALB.ModifyLoadBalancerAttributes(input); err != nil {
		return err
	}
...
func (l *LoadBalancerProvider) setCLBIdleTimeout(loadBalancerName string, idleTimeout int) error {
