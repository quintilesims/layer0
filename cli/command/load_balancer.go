			...
			},
			{
				...
				Flags: []cli.Flag{
					...
					cli.StringFlag{
						...
						Value: dhc.Target,
						Usage: "Health check target in format 'PROTOCOL:PORT' or 'PROTOCOL:PORT/WITH/PATH', used for 'classic' type load balancer",
					},
					cli.StringFlag{
						Name:  "healthcheck-path",
						Value: dhc.Path,
						Usage: "Health check path '/PATH', used for 'application' type load balancer",
					},
					cli.IntFlag{
					...
					cli.StringFlag{
						...
						Usage: "Type of load balancer, either 'classic' or 'application'",
					},
				},
			},
			{
...
func (l *LoadBalancerCommand) create(c *cli.Context) error {
	...
	req := models.CreateLoadBalancerRequest{
		...
		LoadBalancerType: strings.ToLower(c.String("type")),
